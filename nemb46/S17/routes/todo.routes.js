...
cron.schedule("*/1 * * * *", async () => {
  ...
  if (todosToeBeAdded) {
    ...
    // todosToeBeAdded arrays of todo objects
    for (let todo of todosToeBeAdded) {
      try {
        await TodoModel.insertOne(todo);
        passedTodos++;
      } catch {
        failedTodos++
      }
    }
    console.log("Passed Todos", passedTodos, "Failed Todos", failedTodos)
    let report = `<h4>Dear User, </h4>
    <p>The following is the Bulk Todos Updation Report</p>
      <hr>
    <h5 >Passed Todos: <span style="color:green">${passedTodos}</span> </h5>
    <h5 >Failed Todos: <span style="color:red">${failedTodos}</span> </h5>
    <p>Reason for failing may be Todos not validated as per schema</p>`
    /// Call Node Mailer and Send this report 
    // user's email should be dynamic, make is dynamic
